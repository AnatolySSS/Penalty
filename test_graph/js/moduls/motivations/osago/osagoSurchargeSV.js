...
export function osagoSurchargeSV(main_claim_osago_sv, paymentVoluntary, fuExpertise) {
    ...
    //Получение суммы общей выплаты ФО
    ...
    let percentage = 0
    let percentage_paragraph = ""
    let conclusion_paragraph = ""
    let part_satisfaction_helper = ""
    let satisfaction_summ = 0

    //Если по расчету ФУ размер больше, чем по выплатила ФО
    if (fuExpertise[0].summ_with > total_summ_payment) {
        ...
        percentage_paragraph = `<p>Стоимость восстановительного ремонта Транспортного средства Заявителя согласно Экспертному заключению, 
        ...
        conclusion_paragraph = `<p>Таким образом, Финансовая организация, выплатив Заявителю страховое возмещение в размере 
        ${makeRubText_genitive(total_summ_payment)}, исполнила обязательство по Договору ОСАГО в полном объеме, в связи 
        с чем требование Заявителя о взыскании доплаты страхового возмещения не подлежит удовлетворению.</p>`
    }

    let norms_of_law = `<p>Согласно абзацу восьмому пункта 1 статьи 1 Закона № 40-ФЗ договор обязательного страхования 
    ...
    let norms_of_law_2 = `<p>Согласно пункту 41 Постановления Пленума ВС РФ № 58 при осуществлении страхового возмещения в форме страховой 
    выплаты размер расходов на запасные части, в том числе и по договорам обязательного страхования, заключенным начиная 
    с 28.04.2017, определяется с учетом износа комплектующих изделий (деталей, узлов и агрегатов), подлежащих замене при 
    ...
    if (fuExpertise[0].summ_without > fuExpertise[0].summ_market) {
        norms_of_law = `<p>В соответствии с пунктом 1 статьи 12.1 Закона № 40-ФЗ в целях установления обстоятельств 
        причинения вреда транспортному средству, установления повреждений транспортного средства и их причин, технологии, 
        методов и стоимости его восстановительного ремонта проводится независимая техническая экспертиза.</p>
        <p>Согласно части 10 статьи 20 Закона № 123-ФЗ финансовый уполномоченный вправе организовывать проведение независимой 
        экспертизы (оценки) по предмету спора для решения вопросов, связанных с рассмотрением обращения.</p>
        <p>В пункте 3 статьи 12.1 Закона № 40-ФЗ указано, что независимая техническая экспертиза проводится с использованием 
        Единой методики определения размера расходов на восстановительный ремонт в отношении поврежденного транспортного 
        средства, которая утверждается Банком России.</p>`

        norms_of_law_2 = `<p>Поскольку стоимость восстановительного ремонта превышает стоимость Транспортного средства на дату 
        ...
        стоимость.</p>
        <p>Согласно подпункту «а» пункта 16.1 статьи 12 Закона № 40-ФЗ страховое возмещение вреда, причиненного легковому 
        автомобилю, находящемуся в собственности гражданина и зарегистрированному в Российской Федерации, осуществляется 
        путем выдачи суммы страховой выплаты потерпевшему (выгодоприобретателю) в кассе страховщика или перечисления суммы 
        страховой выплаты на банковский счет потерпевшего (выгодоприобретателя) (наличный или безналичный расчет) в случае 
        полной гибели транспортного средства.</p>
        <p>Таким образом, сумма страхового возмещения при полной гибели Транспортного средства составляет 
        ...
        if (date_dtp <= DATE_NEW_OSAGO_METHODOKOGY) {
             //Применяем методику от 19.09.2014 № 432-П
             osago_methodology_paragraph_3 = `<p>Согласно правовой позиции Верховного Суда Российской Федерации, изложенной 
             в Определении судебной коллегии по гражданским делам Верховного Суда Российской Федерации от 28.07.2020 № 
             67-КГ20-6, при разрешении споров о выплате страхового возмещения по риску полной гибели транспортного средства 
             подлежит применению пункт 3.5 Положения № 432-П, согласно которому расхождение в результатах расчетов размера 
             расходов на восстановительный ремонт, выполненных различными специалистами, следует признавать находящимся в 
             пределах статистической достоверности за счет использования различных технологических решений и погрешностей 
             расчета, если оно не превышает 10 процентов.</p>`
        }

        //Если по расчету ФУ размер больше, чем по выплатила ФО
        if ((fuExpertise[0].summ_market - fuExpertise[0].summ_leftovers) > total_summ_payment) {
            percentage = Math.round(((fuExpertise[0].summ_market - fuExpertise[0].summ_leftovers) - total_summ_payment) / total_summ_payment * 100)
            satisfaction_summ = (fuExpertise[0].summ_market - fuExpertise[0].summ_leftovers) - total_summ_payment
            if (main_claim_osago_sv > satisfaction_summ) {
                part_satisfaction_helper = " частичному"
            }

            percentage_paragraph = `<p>Сумма страхового возмещения при полной гибели Транспортного средства согласно экспертному 
            заключению, составленному по инициативе Финансового уполномоченного, превышает размер страхового возмещения, 
            выплаченного Финансовой организацией, на ${makePercentageText_genitive(percentage)}.</p>`

            //Если расхождение превышает 10%
            if (percentage > 10) {
                conclusion_paragraph = `<p>Учитывая вышеизложенное, а также принимая во внимание произведенную Финансовой организацией 
                выплату страхового возмещения в размере ${makeRubText_genitive(total_summ_payment)}, требование Заявителя о доплате 
                страхового возмещения по Договору ОСАГО подлежит удовлетворению${part_satisfaction_helper} в размере 
                ${makeRubText_genitive(satisfaction_summ)} 
                (${makeRubText_genitive(fuExpertise[0].summ_market - fuExpertise[0].summ_leftovers)} - 
                ${makeRubText_genitive(total_summ_payment)}).</p>`
            //Если расхождение не превышает 10%
            } else {
                conclusion_paragraph = `<p>Поскольку указанное расхождение не превышает 10%, то есть находится в пределах статистической 
